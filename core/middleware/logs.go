...
import (
	...
	"os"
	"person/core"
	"person/core/bytebufferpool"
	"person/core/fasttemplate"
	"strconv"
	"strings"
	"sync"
	"sync/atomic"
...
// Loggers creates a new middleware handler
func Loggers(config ...LogConfig) fiber.Handler {
	// Set default config
	cfg := logConfigDefault(config...)
	...
	return func(c *fiber.Ctx) (err error) {
		...
		if cfg.Format == LogConfigDefault.Format {
			...
			fmt.Println(fmt.Sprintf("%s |%s %3d %s| %7v | %15s |%s %-7s %s| %-"+errPaddingStr+"s %s\n",
				timestamp.Load().(string),
				stop.Sub(start).Round(time.Millisecond),
				c.IP(),
				c.Path(),
			...
			core.LOGGER.Info(string(buf.Bytes()))
